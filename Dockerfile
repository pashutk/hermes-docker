...
RUN python3 -c "from pathlib import Path; p = Path('/opt/hermes/cron/scheduler.py'); s = p.read_text(); old = 'argv = [python_exe, str(path)]'; new = 'argv = [\"/usr/local/bin/hermes-cron-python\", str(path)]'; assert old in s; p.write_text(s.replace(old, new, 1))"
