...
RUN python3 -c "from pathlib import Path; p = Path('/opt/hermes/cron/scheduler.py'); s = p.read_text(); old = '[python_exe, str(path)]'; new = '["/usr/local/bin/hermes-cron-python", str(path)]'; assert old in s or new in s, f'Pattern {old!r} not found in scheduler.py'; p.write_text(s.replace(old, new, 1))"
